...
CANONICAL_BRAND_MAP = {
    "asus": "ASUS",
    # Vendor product lines / wholly-owned sub-brands that the extractor sometimes
    # emitted as standalone brands even though the parent vendor is present as a
    # separate node in the same dataset. Left unmerged, each one creates a second
    # node for a single commercial entity and splits its exposure weight. In every
    # case the specific model is preserved in the record's `product` field.
    "snapdragon": "Qualcomm",          # Qualcomm's system-on-chip family
    "omen": "HP",                      # HP's gaming sub-brand ("looking at their Omen line")
    "hp omen": "HP",                   # ditto ("That's why it's called HP Omen")
    "xps": "Dell",                     # Dell's laptop line ("Dell had like the XPS")
    "playstation": "Sony",             # Sony's console brand
    "xbox": "Microsoft",               # Microsoft's console brand
    "xbox game studios": "Microsoft",  # Microsoft first-party publishing label
    "crucial": "Micron",               # Micron's consumer retail brand
}
...
class NetworkAnalyzer:
    ...
    def compute_all_metrics(self) -> Dict[str, Any]:
        ...
        #
        # On this graph the pure-Python power iteration reliably fails to converge
        # (the weighted spectrum is dominated by a few very high-view hubs), so the
        # scipy-backed dense eigensolver is the path that actually produces the
        # reported values. Non-convergence is therefore an EXPECTED numerical
        # condition and is handled; anything else (most importantly a missing scipy,
        # which makes eigenvector_centrality_numpy raise ImportError) is a hard
        # failure and must not be papered over. A previous revision fell back to a
        # uniform 1/|V| for every node, which silently produced a plausible-looking
        # but meaningless centrality column.
        try:
            eigenvector = nx.eigenvector_centrality(
                self.undirected_G, weight="weight", max_iter=1000, tol=1e-6
            )
        except nx.PowerIterationFailedConvergence:
            print("[*] Eigenvector power iteration did not converge; "
                  "using the dense scipy eigensolver instead.")
            eigenvector = nx.eigenvector_centrality_numpy(self.undirected_G, weight="weight")
            # eigenvector_centrality_numpy may return numpy scalars; normalise to float.
            eigenvector = {n: float(v) for n, v in eigenvector.items()}

        if len(set(round(v, 12) for v in eigenvector.values())) == 1 and self.G.number_of_nodes() > 1:
            raise RuntimeError(
                "Eigenvector centrality is uniform across all nodes, which indicates a "
                "degenerate solve rather than a real result. Refusing to emit it."
            )
        ...
        # PageRank & Closeness.
        # nx.pagerank is scipy-backed. As above, a genuine convergence failure is
        # surfaced rather than replaced with a uniform 1/|V| distribution.
        try:
            pagerank = nx.pagerank(self.G, weight="weight", alpha=0.85)
        except nx.PowerIterationFailedConvergence as exc:
            raise RuntimeError(
                "PageRank failed to converge on the creator-brand graph; the reported "
                "centralities would be meaningless. Inspect the edge weights."
            ) from exc
    ...
    def detect_communities(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Older NetworkX without louvain_communities: greedy modularity is an
            # acceptable substitute algorithm and is reported as such.
            print("[*] louvain_communities unavailable in this NetworkX version; "
                  "falling back to greedy modularity maximisation.")
