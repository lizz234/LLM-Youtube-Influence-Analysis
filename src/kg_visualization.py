...
class KnowledgeGraphVisualizer:
    ...
    def generate_publication_figures(self) -> Dict[str, Path]:
        ...
        diffusion_res = self.analyzer.simulate_information_diffusion()
        ...
        for strat, data in diffusion_res.items():
            curve = data.get("cascade_curve", [])
            steps = list(range(len(curve)))
            lbl = labels.get(strat, strat)
            plt.plot(steps, curve, marker='o', linewidth=2.5, color=colors.get(strat, '#6B7280'), label=f"{lbl} (Final Reach: {data.get('mean_final_reach', 0):.1f} nodes)")

        plt.title("Information Diffusion Dynamics: Independent Cascade Model (ICM)", pad=12, fontweight='bold')
        plt.xlabel("Diffusion Time Step ($t$)")
        plt.ylabel("Cumulative Activated Nodes ($|A_t|$)")
