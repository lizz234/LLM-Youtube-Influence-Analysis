...
# Reconstruct Graph from the single source of truth (NetworkAnalyzer) so that all
# figures are generated from exactly the same graph used for the reported metrics
# (identical node/edge filtering, self-edge removal, and entity normalization).
from src.network_analysis import NetworkAnalyzer
_analyzer = NetworkAnalyzer()
G = _analyzer.G
undirected_G = _analyzer.undirected_G
...
creators_log = [
    (c["node"], c["score"]) for c in metrics_data.get("top_creators_by_eigenvector", [])[:6]
]
...
_llm = eval_data["llm_gemini_2_5"]
_base = eval_data["rule_based_keyword_baseline"]
llm_scores = [round(_llm["precision"], 3), round(_llm["recall"], 3), round(_llm["f1_score"], 3)]
baseline_scores = [round(_base["precision"], 3), round(_base["recall"], 3), round(_base["f1_score"], 3)]
